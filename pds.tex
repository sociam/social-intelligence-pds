...
\section{Challenges Supporting Information Activities in PDSes}
...
The second challenge pertains to tackling the complexities of data security and maintenance for individuals who might have little or no experience in the intricacies of data magagement.  The current state of data management means that often merely ensuring that one's personal data is not lost to hardware and software failure, malicious attacks, or safely migrated to new platforms and devices requires considerable time and expertise.  This motivates people to essentially outsource maintenance of data to third parties, such as cloud data providers we describe later.  In order to achieve the goals of PDSes, thus, which is increased self-autonomy, the platform itself must directly support and automate common maintenance activities.

The third arises from the proposal for PDSes to be \emph{user-centric}, which proposes that a person's data be kept in a consolidated under the user's control instead of necessarily placed entirely  among a potentially large number of third party service-providers and apps.  Note that 

 from a shift from service-provider-centric challenge is to provide enough flexibility to accommodate future applications

for data robustness against physical device failure or loss, this requires PDSes to fundamentally allow people to freely replicate their 

or ensuring continued readability and usabililty as the hardware and software platforms that might be used with it changes.  The second is essentially to provide flexibility to adapt to future needs nd applications as they arise, so that PDSes continue to be useful as time passes.  A effort at doing

The second is in self-maintenance; 

is the consideration of factors involved in the long-term maintenance and evolution of personal data collections.

\subsubsection{Proactive support: Context-sensitivity and automation }
